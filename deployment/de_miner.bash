#!/bin/bash
echo "upgrade miner(rc.local): wget https://raw.githubusercontent.com/xfstudio/nodejs-pool/master/deployment/de_miner.bash -o /root/de_miner.bash -N && bash /root/de_miner.bash"
...
if [ $(sed -n '16p' /etc/salt/minion)x = "master: $HOST_MASTER"x ];
then
...
then 
~/xmrMinerProject/build/xmrMiner -o stratum+tcp://$POOL_HOST:$POOL_PORT -u $MINER_WALLET -p worker -z 0 -B 
else 
